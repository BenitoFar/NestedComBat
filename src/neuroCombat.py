...
def neuroCombat(dat,
    ...
    if ref_batch is None:
        ...
        if ref_indices.shape[0]==0:
            ...
            encoding_order = np.unique(covars[:,batch_col],return_inverse=True)[0]
            covars[:,batch_col] = np.unique(covars[:,batch_col],return_inverse=True)[-1]
    ...
    estimates = {'batches': info_dict['batch_levels'], 'encoding_order': info_dict['encoding_order'], 'var.pooled': v_pool, 'stand.mean': s_mean, 'mod.mean': mod_mean, 'gamma.star': gamma_star, 'delta.star': delta_star}
